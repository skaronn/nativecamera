#ifndef STATIC_LINK
#define IMPLEMENT_API
#endif

#if defined(HX_WINDOWS) || defined(HX_MACOS) || defined(HX_LINUX)
#define NEKO_COMPATIBLE
#endif

#include <hx/CFFI.h>
#include "Utils.h"

using namespace nativecamera;

static value nativecamera_device_number() {
	int returnValue = deviceNumber();
	return alloc_int(returnValue);
}
DEFINE_PRIM (nativecamera_device_number, 1);

static value nativecamera_initialize() {
	int returnValue = deviceNumber();
	return alloc_int(returnValue);
}
DEFINE_PRIM (nativecamera_initialize, 1);


static value nativecamera_sample_method (value inputValue) {
	
	int returnValue = SampleMethod(val_int(inputValue));
	return alloc_int(returnValue);
	
}
DEFINE_PRIM (nativecamera_sample_method, 1);


extern "C" void nativecamera_main () {
	
	val_int(0); // Fix Neko init
	
}
DEFINE_ENTRY_POINT (nativecamera_main);


extern "C" int nativecamera_register_prims () { return 0; }